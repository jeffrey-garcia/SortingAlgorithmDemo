...
/**
 * The <code>InsertionSort</code> class is a class implementing the 
 * SortAlgorithm which performs sorting operation using the insertion 
 * sort algorithm.
 * 
 * <p>
 *
 ...
 */
public class InsertionSort implements Runnable, SortAlgorithm {
	/**
	...
	public void sort() {
		try {
			for (int index=1; index<c.length; index++) {
				Float key = (Float)c[index];
				int position = index;
				
				comparisonCount ++;
				while (position>0 && key.compareTo((Float)c[position-1])<0) {
					//System.out.println("[Swap] " + c[position-1] + " with " + c[position]); //Debugger
				...
				//System.out.println("[Swap] " + key + " with " + c[position]); //Debugger
				c[position] = key;
				swapCount ++;
				
				//System.out.print("Sort Move --- " + index); //Debugger showing no. of sort
